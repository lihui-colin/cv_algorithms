...
inline double SignedDistance(double x, double y) {
    ...
    double hole = std::hypot(x + 5, y - 1) - 5;
        ...
        for (int x = 0; x < width; ++x) {
            ...
            for (int j = 0; j < samples; ++j)
                for (int i = 0; i < samples; ++i) {
                    ...
                    double d = SignedDistance(qx, qy) * scale;
                    sum += 25 + 205 * 0.5 * (1 + std::erf(d / (0.9 * std::sqrt(2.0))));
                }
